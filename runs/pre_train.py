...
def pretrain(args):
    ...
    for epoch in range(args.n_iters):
        ...
        for batch_idx, (ptclouds, labels) in enumerate(TRAIN_LOADER):
            ...
            remaining_time = batch_time*(args.n_iters*len(TRAIN_LOADER)-global_iter)
            hours, seconds = divmod(remaining_time, 3600)
            minutes, seconds = divmod(seconds, 60)

            if batch_idx % 20==0:
                WRITER.add_scalar('Train/loss', loss, global_iter)
                logger.cprint('=====[Train] Epoch: [%d/%d] | Iter: [%d/%d] | Loss: %.4f | Batch Time: %.2f s | Remaining Time: %d h %d mins=====' %
                (epoch, args.n_iters, batch_idx, len(TRAIN_LOADER), loss.item(), batch_time, hours, minutes))
